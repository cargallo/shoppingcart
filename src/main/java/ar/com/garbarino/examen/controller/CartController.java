...
public class CartController {
	...
	public ResponseEntity<?> createCart(@Valid @RequestBody CartRequest cartRequest) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{cartId}").buildAndExpand(cart.getId())
				.toUri();
		return ResponseEntity.created(location).body(new ApiResponse(true, "Cart Created Successfully"));
	}

	@PostMapping("/{cartId}/products")
	public ResponseEntity<?> addProduct(@PathVariable(required = true) Long cartId,
			@RequestBody CartProductRequest cartProductRequest) {
		cartService.addProduct(cartId, cartProductRequest);
		return ResponseEntity.ok().body(new ApiResponse(true, "Product Added Successfully"));
	}
